    ...
    IMediaGroupService _mediaGroupService)
{
    ...
    public async Task ResponseCallAdmins(Users dbUser, Message message)
    {
        ...
        {
            var sb = new StringBuilder();
            var admins = await _botClient.GetChatAdministrators(message.Chat.Id).ConfigureAwait(false);
            ...
            try
            {
                await _botClient.EditMessageText(msg, $"用户 {dbUser} 呼叫群管理", ParseMode.Html).ConfigureAwait(false);
            }
            catch (Exception)
            {
    ...
    public async Task QResponseCancel(CallbackQuery query, string[] args)
    {
        ...
        await _botClient.AutoReply(text, query).ConfigureAwait(false);
        await _botClient.EditMessageText(query.Message!, text, replyMarkup: null).ConfigureAwait(false);
    }
    ...
    public async Task QResponseSetRandomPost(Users dbUser, CallbackQuery callbackQuery, string[] args)
    {
        ...
        await _botClient.EditMessageText(callbackQuery.Message!, text, replyMarkup: keyboard).ConfigureAwait(false);
    }
    ...
    {
        var keyboard = _markupHelperService.RandomPostMenuKeyboard(dbUser);
        await _botClient.EditMessageText(callbackQuery.Message!, "请选择感兴趣的稿件标签", replyMarkup: keyboard).ConfigureAwait(false);
    }
    ...
    public async Task QGetRandomPost(Users dbUser, CallbackQuery callbackQuery, string[] args)
    {
        ...
        if (randomPost != null)
        {
            ...
            if (randomPost.IsMediaGroup)
            {
                ...
                var messages = await _botClient.SendMediaGroup(chat, group).ConfigureAwait(false);
                await _botClient.SendMessage(chat, "随机稿件操作", replyMarkup: keyboard).ConfigureAwait(false);
            ...
            else
            {
                ...
                if (handler == null)
                {
                    await _botClient.AutoReply($"不支持的稿件类型: {randomPost.PostType}", callbackQuery, true).ConfigureAwait(false);
                    await _botClient.EditMessageText(callbackQuery.Message!, $"不支持的稿件类型: {randomPost.PostType}").ConfigureAwait(false);
                    return;
                }
            ...
            if (args.Length > 3 && long.TryParse(args[3], out long msgId))
            {
                ...
                await _botClient.EditMessageReplyMarkup(callbackQuery.Message!, kbd).ConfigureAwait(false);
            }
            else
            {
        ...
        {
            await _botClient.EditMessageText(callbackQuery.Message!, "无可用稿件", replyMarkup: null).ConfigureAwait(false);
        }
    }
    ...
    public async Task GetIpInfo(Message message, string[] args)
    {
        ...
        await _botClient.AutoReply(sb.ToString(), message, ParseMode.Html).ConfigureAwait(false);
    }
}
