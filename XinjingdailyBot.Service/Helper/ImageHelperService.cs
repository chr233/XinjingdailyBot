    ...
    IHttpHelperService _httpHelperService) : IImageHelperService
{
    ...
    private async Task<double?> CalcDocumentImageRatio(Message message, Document document)
    {
        ...
        var tipsMsg = await _botClient.AutoReply("正在处理图片文件，请稍候...", message).ConfigureAwait(false);

        var file = await _botClient.GetFile(document.FileId).ConfigureAwait(false);
