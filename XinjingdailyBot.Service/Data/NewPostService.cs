...
{
    [AppService(typeof(INewPostService), LifeTime.Singleton)]
    public sealed class NewPostService : BaseService<NewPosts>, INewPostService
    {
        ...
        public async Task HandleTextPosts(Users dbUser, Message message)
        {
            ...
            {
                await _botClient.AutoReplyAsync($"文本长度超过上限 {INewPostService.MaxPostText}, 无法创建投稿", message);
                return;
            }
            ...
            if (message.ForwardFromChat?.Type == ChatType.Channel)
            {
                ...
                channelOption = await _channelOptionService.FetchChannelOption(channelId, message.ForwardFromChat.Username, message.ForwardFromChat.Title);
            }
        ...
        public async Task<NewPosts?> FetchPostFromReviewCallbackQuery(CallbackQuery query)
        {
        ...
        public async Task<NewPosts?> FetchPostFromConfirmCallbackQuery(CallbackQuery query)
        {
            if (query.Message == null)
            {
                return null;
